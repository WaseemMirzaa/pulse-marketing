...
function jas_gecko_vc_map_meta_slider() {
	...
	vc_map(
		array(
			'name'     => esc_html__( 'Meta Slider', 'gecko' ),
			'description' => esc_html__( 'Meta Slider element', 'gecko' ),
			'base'     => 'metaslider',
			'icon'     => 'pe-7s-albums',
			...
			'params'   => array(
				array(
					'param_name' => 'id',
					'heading'    => esc_html__( 'Choose slide', 'gecko' ),
					'type'       => 'dropdown',
					'value'      => $slide,
				)
			)
...
add_action( 'vc_before_init', 'jas_gecko_vc_map_meta_slider' );