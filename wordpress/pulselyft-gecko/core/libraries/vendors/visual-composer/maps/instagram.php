...
function jas_gecko_vc_map_instagram() {
	vc_map(
		array(
			...
			'description' 	=> esc_html__( 'Instagram element', 'gecko' ),
			'base'     		=> 'jas_instagram',
			'icon'     		=> 'pe-7s-share',
			...
			'params'  		=> array(
				array(
					...
					'description' => sprintf( wp_kses_post( 'Lookup User ID <a target="_blank" href="%s">here</a>', 'gecko' ), 'https://codeofaninja.com/tools/find-instagram-user-id' ),
					'type'        => 'textfield',
				),
				array(
				...
				array(
					'param_name'  => 'size',
					'heading'     => esc_html__( 'Image size', 'gecko' ),
					'type'        => 'dropdown',
					'value'       => array(
						esc_html__( '150x150', 'gecko' ) => 'thumbnail',
						esc_html__( '320x320', 'gecko' ) => 'low',
						esc_html__( '640x640', 'gecko' ) => 'standard',
					),
					'std' => 'low',
				),
				array(
