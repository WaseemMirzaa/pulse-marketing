...
if ( $style == '1' || $style == '3' ) {
	$attr = '{"slidesToShow": 1,"slidesToScroll": 1,"asNavFor": ".p-nav","fade":true,"adaptiveHeight":true'. ( is_rtl() ? ',"rtl":true' : '' ) .'}';
} else {
	$attr = '{"slidesToShow": 3,"slidesToScroll": 1,"centerMode":true, "responsive":[{"breakpoint": 960,"settings":{"slidesToShow": 2, "centerMode":false}},{"breakpoint": 480,"settings":{"slidesToShow": 1, "centerMode":false}}]'. ( is_rtl() ? ',"rtl":true' : '' ) .'}';
}
?>
	...
	<div class="p-thumb images jas-carousel" data-slick='<?php echo esc_attr( $attr ); ?>'>
		<?php
			if ( has_post_thumbnail() ) {
				...
				$html  = '<div data-thumb="' . get_the_post_thumbnail_url( $post->ID, 'shop_thumbnail' ) . '" class="p-item woocommerce-product-gallery__image' . $zoom . '"><a href="' . esc_url( $full_size_image[0] ) . '" class="woocommerce-product-gallery__trigger">';
