...
<form class="variations_form cart" method="post" enctype='multipart/form-data' data-product_id="<?php echo absint( $product->get_id() ); ?>" data-product_variations="<?php echo esc_attr( json_encode( $available_variations ) ) ?>">
	...
	<?php if ( empty( $available_variations ) && false !== $available_variations ) : ?>
		<p class="stock out-of-stock"><?php _e( 'This product is currently out of stock and unavailable.', 'gecko' ); ?></p>
	<?php else : ?>
			...
			<?php foreach ( $attributes as $attribute_name => $options ) : ?>
				<div class="product-attribute">
					...
					<?php
						...
						echo end( $attribute_keys ) === $attribute_name ? '<a class="reset_variations" href="#">' . __( 'Clear selection', 'gecko' ) . '</a>' : '';
					?>
				</div>
			<?php endforeach;?>
		</div>
										...
										for ( var i = 1; i < attributes.length; i++ ) {
											...
											if ( ! check ) {
												if ( $( 'select#' + attribute + ' option' ).length > 1 ) {
													...
													$( 'ul#' + attribute + ' li' ).each( function() {
														if ( value == $( this ).attr( 'option-value' ) ) {
															$( this ).removeClass( 'disable' );
														} else {
													...
													for ( var i = 0; i < enable_values.length; i++ ) {
														$( 'ul#' + attribute + ' li[option-value="' + enable_values[i] + '"]' ).removeClass( 'disable' );
													}
