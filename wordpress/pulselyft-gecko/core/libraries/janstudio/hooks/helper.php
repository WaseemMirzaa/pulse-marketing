			...
			$output .= '<a class="db" href="' . esc_url( home_url( '/' ) ) . '">';
				if ( ! cs_get_option( 'header-transparent' ) ) {
					if ( cs_get_option( 'logo' ) ) {
						...
						$output .= '<img class="regular-logo" src="' . esc_url( $logo[0] ) . '" width="' . esc_attr( $logo[1] ? $logo[1] : '' ) . '" height="' . esc_attr( $logo[2] ? $logo[2] : '' ) . '" alt="' . get_bloginfo( 'name' ) . '" />';
					} else {
					...
					if ( cs_get_option( 'logo-retina' ) ) {
						...
						$output .= '<img class="retina-logo" src="' . esc_url( $logo_retina[0] ) . '" width="' . esc_attr( $logo_retina[1] ? $logo_retina[1] / 2 : '' ) . '" height="' . esc_attr( $logo_retina[2] ? $logo_retina[2] / 2 : '' ) . '" alt="' . get_bloginfo( 'name' ) . '" />';
					} else {
	...
	function jas_gecko_post_meta() {
		...
		if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			...
			if ( '1' === $comments_number ) {
				$output .= '<span class="comment-number pr"><a href="' . get_comments_link() . '">' . sprintf( _x( '1 Comment', 'comments title', 'gecko' ), get_the_title() );
				$output .= '</a></span>';
			} else {
		...
		if ( ! is_front_page() ) {
			...
			if ( is_home() ) {
				...
				if ( ! empty( $portfolio_title ) ) {
					$output .= '<li class="fl current">' . $portfolio_title . '</li>';
				} else {
					$output .= '<li class="fl current">' . esc_html__( 'Portfolio', 'gecko' ) . '</li>';
				}
			...
			<div class="social-share">
				<div class="jas-social">
					...
					<?php if ( cs_get_option( 'vk-share' ) ) { ?>
						<a title="<?php echo esc_html__( 'Share this post on VK', 'gecko' ); ?>" class="cb vk" data-content="<?php echo esc_url( $src[0] ); ?>" href="//vk.com/share.php?url=<?php esc_url( the_permalink() ); ?>" onclick="javascript:window.open(this.href, '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=600,width=540');return false;">
							<i class="fa fa-vk"></i>
						</a>
					<?php } ?>
					<?php if ( cs_get_option( 'linkedin-share' ) ) { ?>
						<a title="<?php echo esc_html__( 'Share this post on LinkedIn', 'gecko' ); ?>" class="cb linkedin" data-content="<?php echo esc_url( $src[0] ); ?>" href="https://www.linkedin.com/sharing/share-offsite/?url=<?php esc_url( the_permalink() ); ?>" onclick="javascript:window.open(this.href, '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=600,width=540');return false;">
							<i class="fa fa-linkedin"></i>
						</a>
					<?php } ?>
					<?php if ( cs_get_option( 'whatsapp-share' ) ) { ?>
						<a title="<?php echo esc_html__( 'Share this post on WhatsApp', 'gecko' ); ?>" class="cb whatsapp" data-content="<?php echo esc_url( $src[0] ); ?>" href="whatsapp://send?text=<?php esc_url( the_permalink() ); ?>" onclick="javascript:window.open(this.href, '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=600,width=540');return false;">
							<i class="fa fa-whatsapp"></i>
						</a>
					<?php } ?>
					<?php if ( cs_get_option( 'telegram-share' ) ) { ?>
						<a title="<?php echo esc_html__( 'Share this post on Telegram', 'gecko' ); ?>" class="cb telegram" data-content="<?php echo esc_url( $src[0] ); ?>" href="https://t.me/share/url?url=<?php esc_url( the_permalink() ); ?>" onclick="javascript:window.open(this.href, '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=600,width=540');return false;">
				...
				$output .= '<div class="jas-carousel" data-slick=\'{"slidesToShow": 3,"slidesToScroll": 1, "responsive":[{"breakpoint": 960,"settings":{"slidesToShow": 2}},{"breakpoint": 480,"settings":{"slidesToShow": 1}}]'. ( is_rtl() ? ',"rtl":true' : '' ) .'}\'>';
					while ( $related->have_posts() ) :
						...
						$output .= '<div class="item">';
							...
							$output .= '<span class="f__libre">' . get_the_time( 'F j, Y' ) . '</span>';
						$output .= '</div>';
					endwhile;
				$output .= '</div>';	
			$output .= '</div>';
...
if ( ! function_exists( 'jas_gecko_custom_data_js' ) ) {
	function jas_gecko_custom_data_js() {
		$data = array(
			...
			'popup_remove' => esc_html__( 'Removed from the cart', 'gecko' ),
			'popup_undo'   => esc_html__( 'Undo?', 'gecko' ),
		);
		...
		// Live Search
		$data['live_search'] = cs_get_option( 'header-live-search' );
