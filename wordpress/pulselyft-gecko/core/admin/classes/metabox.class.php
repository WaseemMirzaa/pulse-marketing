...
class CSFramework_Metabox extends CSFramework_Abstract{
  ...
  public function __construct( $options ){
    ...
    if( ! empty( $this->options ) ) {
      $this->addAction( 'add_meta'.'_'.'boxes', 'add_metabox' );
  ...
  // add metabox
    ...
    foreach ( $this->options as $value ) {
      call_user_func( 'add_meta'.'_'.'box', $value['id'], $value['title'], array( &$this, 'render_meta_box_content' ), $value['post_type'], $value['context'], $value['priority'], $value );
    }
  ...
  public function render_meta_box_content( $post, $callback ) {
    ...
    $cs_errors    = !empty($transient['errors']) ? $transient['errors'] : [];
    ...
    $section_id   = ( ! empty( $transient['ids'][$unique] ) ) ? $transient['ids'][$unique] : $section_name;
      ...
      if( !in_array($section_id, [
          ...
          '_custom_wc_options',
          '_custom_wc_thumb_options',
      ]) ){
          $section_id = '_custom_page_options';
      }

    echo '<div class="cs-framework cs-metabox-framework">';

      echo '<input type="hidden" name="cs_section_id['. $unique .']" class="cs-reset" value="'. esc_attr($section_id) .'">';
