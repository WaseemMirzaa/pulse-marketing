...
class CSFramework extends CSFramework_Abstract {
  ...
  public function __construct( $settings, $options ) {
    ...
    if( ! empty( $this->options ) ) {
      ...
      $this->addAction( 'admin_menu', 'admin_menu', 10 );
  ...
  public function validate_save( $request ) {
    ...
    if ( isset( $request['import'] ) && ! empty( $request['import'] ) ) {
      ...
      $add_errors[] = $this->add_settings_error( esc_html__( 'Success. Imported backup options.', 'gecko' ), 'updated' );
    }

    // reset all options
    if ( isset( $request['resetall'] ) ) {
      $add_errors[] = $this->add_settings_error( esc_html__( 'Default options restored.', 'gecko' ), 'updated' );
      return;
    }
    ...
    if ( isset( $request['reset'] ) && ! empty( $section_id ) ) {
      ...
      $add_errors[] = $this->add_settings_error( esc_html__( 'Default options restored for only this section.', 'gecko' ), 'updated' );
    }
  ...
  public function admin_page() {
    ...
    $section_id = ( ! empty( $transient['section_id'] ) ) ? $transient['section_id'] : $this->sections[0]['name'];
      ...
      $section_id = sanitize_key( cs_get_var( 'cs-section', $section_id ) );
    ...
    echo '<div class="cs-framework cs-option-framework">';
      ...
      echo '<input type="hidden" class="cs-reset" name="cs_section_id" value="'. esc_attr($section_id) .'" />';
      ...
      echo '<h1><img src="' . CS_URI . '/assets/images/janstudio-logo.png" width="73" height="41" alt="JanStudio" /> <small>' . JAS_GECKO_VERSION . '</small></h1>';
      echo '<fieldset>';
      if ( $this->settings['ajax_save'] ){
          echo '<span id="cs-save-ajax">'. esc_html__( 'Settings saved.', 'gecko' ) .'</span>';
      }

      submit_button( esc_html__( 'Save', 'gecko' ), 'primary', 'save', false, array( 'data-ajax' => $this->settings['ajax_save'], 'data-save' => esc_html__( 'Saving...', 'gecko' ) ) );
      submit_button( esc_html__( 'Restore', 'gecko' ), 'secondary cs-restore cs-reset-confirm', $this->unique .'[reset]', false );
      if( $this->settings['show_reset_all'] ) {
        submit_button( __( 'Reset All Options', 'gecko' ), 'secondary cs-restore cs-warning-primary cs-reset-confirm', $this->unique .'[resetall]', false );
      }
      echo '</fieldset>';
      echo ( empty( $has_nav ) ) ? '<a href="#" class="cs-expand-all"><i class="fa fa-eye-slash"></i> '. esc_html__( 'show all options', 'gecko' ) .'</a>' : '';
